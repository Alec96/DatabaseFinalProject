...
scrapeFullLoc <- function (x) {
  options(timeout= 4000000)
  climbFullLoc <- html_text(html_node(read_html(x), '.max-height-xs-600:nth-child(2) .fr-view'))
  return(climbLoc)
}

html_text(html_node(read_html("https://www.mountainproject.com/route/105945043/ride-the-lightning"), '.mb-half.text-warm'))
```
